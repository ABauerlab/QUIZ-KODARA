...
function mensagemWhats(lead: Lead, valor: number | null) {
  const linhas = [
    ...
    `Valor estimado: ${valor ? formatBRL(valor) : 'sob consulta'}`,
  ]
  return encodeURIComponent(linhas.join('\n'))
}

export default function Final({ lead, valor, precoUnitario }: Props) {
  ...
  async function salvar() {
    ...
    try {
      ...
      setSalvo(true)
      pixel.quizCompleted(valor, { content_name: lead.tipo_peca ?? 'private_label' })
    } catch {
      setErroSalvar(true)
    } finally {
      setSalvando(false)
  ...
  useEffect(() => {
    // Guarda contra o duplo mount do StrictMode em dev, pra nao gravar 2 leads.
    if (tentativa.current > 0) return
    pixel.initiateCheckout(valor, { content_name: lead.tipo_peca ?? 'private_label' })
    void salvar()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  // O lead nunca fica preso: depois de uma tentativa que falhou, o WhatsApp libera.
  const liberado = salvo || (erroSalvar && !salvando)
  ...
  const entrada = valor ? valor / 2 : null

  function irProWhats() {
    pixel.whatsappRedirect(valor, { content_name: lead.tipo_peca ?? 'private_label' })
    window.location.href = `https://wa.me/${env.whatsapp}?text=${mensagemWhats(lead, valor)}`
  }
  ...
  return (
    <div className="grid gap-4 pb-8">
      ...
      <div className="rounded-2xl border border-acid/40 bg-acid/5 p-4">
        {valor ? (
          <>
            <p className="text-sm text-mute">Valor total estimado</p>
            <p className="text-3xl font-black text-acid">{formatBRL(valor)}</p>
            {precoUnitario && (
      ...
      <div className="sticky bottom-0 -mx-4 border-t border-line bg-ink/95 px-4 py-3 backdrop-blur">
        <button className="btn-primary" disabled={!liberado} onClick={irProWhats}>
          {liberado ? 'Falar agora no WhatsApp da Kodara' : 'Salvando seu briefing...'}
        </button>
        {erroSalvar && (
